...
const cards = () => {
  ...
  const recipesCards = () => {
    return recettes.map((recipe) => (
      <div
        key={recipe.id}
        className="card  flex flex-col bg-white border border-gray-200 rounded-lg shadow-lg hover:scale-105 transition-transform duration-300 ease-in-out transition-shadow  duration-300 ease-in-"
      >
        <img
        ...
        <p className="text-base text-gray-700 flex-grow m-2 mt-2 mb-7 line-clamp-2">
          {recipe.description}
        </p>
        <button
